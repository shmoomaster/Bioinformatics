...
# Remove the seqence itself and replace with ASV
```{r}
...
# remove mitochonria and chloroplast matches, remove all non bacterial sequences
```{r}
...
# create a barplot relative abundance
```{r}
...
```{r}
# extract sample information form the physeq object
...
# merge alpha diversity data and samples data
alpha <- merge(alpha, samples, by = "sample")
```

# perform statistics based on subject
```{r}
# Perform t/wilcox test for each biodiversity index
...
print(test_shannon)
```
# perform statistics based on reported.antibioics.usage
```{r}
# Perform t/wilcox test for each biodiversity index
